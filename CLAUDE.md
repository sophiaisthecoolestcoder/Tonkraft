...
**Hard rules**:
- **DRY / Single source of truth (NON-NEGOTIABLE — prevents design drift)**: every colour, font, spacing, radius, shadow and timing value is a **design token** declared ONCE in the `:root`/theme blocks of `css/styles.css`; components reference `var(--token)` and NEVER hard-code a hex/rgb/font-stack/raw colour. Widgets are centralized, reusable classes — per-instance differences use modifiers (e.g. `.hero__word--compact`) or a passed token (e.g. `--tone-color`), never a copied/re-tweaked implementation. No design-bearing inline `style=` in HTML (use a class, a modifier, or the `.u-mt-*`/`.u-mb-*` spacing utilities). JS reads palette/values from the CSS tokens at runtime (`getComputedStyle`), never re-declaring them. Theming = override semantic tokens in one block (see `[data-theme="dark"]` scaffold). See `.specify/memory/constitution.md` §I. Litmus test before committing: *is there exactly one place to change this value?*
